	...
	if (damagingEntity.getComponent("equippable")?.getEquipment("Mainhand")) {
		...
		switch (item.typeId) {
			...
			case "billey:shark_tooth_sword":
				...
				if (lol.isInWater)
					lol.runCommand("execute @s ^^^2 damage @e[tag=!tamed,type=!xp_orb,type=!item,family=!inanimate,family=!wolf,tag=!immunetosharksword,r=2.25] 11 entity_attack entity @s");
				else
					lol.runCommand("execute @s ^^^2 damage @e[tag=!tamed,type=!xp_orb,type=!item,family=!inanimate,family=!wolf,tag=!immunetosharksword,r=3.25] 11 entity_attack entity @s");
